 ...
 *
 * Portado desde CyberQRM (backend/src/services/simulationEngine.ts, commit b074b3d).
 * Lógica sin cambios; única adaptación: `uuid` reemplazado por `crypto.randomUUID()`
 * (Web API nativa del runtime de Convex, evita agregar una dependencia npm).
 *
 * Implementa muestreo Triangular, Lognormal y Point, y corre el árbol de cálculo
 * FAIR para producir una distribución de valores ALE.
 */
...
function computeConvergence(samples: Float64Array) {
 ...
 */
export function runSensitivityAnalysis(scenario: RiskScenario, baseALE: number): SensitivityResult[] {
  ...
  return results.sort((a, b) => Math.abs(b.upperALE - b.lowerALE) - Math.abs(a.upperALE - a.lowerALE));
}

function keyLabel(key: FAIRComponentKey): string {
  switch (key) {
    case "tef":
      return "Threat Event Frequency";
    case "vulnerability":
      return "Vulnerability";
    case "assetValue":
      return "Asset Value";
    case "lei":
      return "Loss Event Impact";
  }
}
...
export function projectControlImpact(
  ...
  const paybackPeriodMonths = monthlyALEReduction > 0 ? Math.ceil(implementationCost / monthlyALEReduction) : 999;
...
function cloneWithEffectiveness(scenario: RiskScenario, key: FAIRComponentKey, effectiveness: number): RiskScenario {
  const clone: RiskScenario = JSON.parse(JSON.stringify(scenario));
